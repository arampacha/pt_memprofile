...
__all__ = ['MemHooks', 'plot_log', 'reset_grad', 'memprofile', 'forward_and_loss', 'memprofile', 'simple_model',
           'MemProfileCallback', 'MemStatsCallback']
...
# Cell
def memprofile(model:nn.Module, xb:Tensor, yb:Tensor, loss_func=CrossEntropyLossFlat(), plot=True, label=None):
...
# Cell
def forward_and_loss(model, xb, yb, loss_func):
    out = model(xb)
